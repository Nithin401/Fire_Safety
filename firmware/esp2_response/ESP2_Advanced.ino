...
void relayOFF() {
  if (RELAY_ACTIVE_LOW) digitalWrite(RELAY_GPIO, HIGH);
  else digitalWrite(RELAY_GPIO, LOW);
}

void relayON() {
  if (RELAY_ACTIVE_LOW) digitalWrite(RELAY_GPIO, LOW);
  else digitalWrite(RELAY_GPIO, HIGH);
}

int convertAngle(int receivedAngle) {
  receivedAngle = constrain(receivedAngle, 0, 180);
  int outputAngle = REVERSE_SERVO ? (180 - receivedAngle) : receivedAngle;
